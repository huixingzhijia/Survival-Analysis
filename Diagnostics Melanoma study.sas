****************************************************
/* The following data set is a cohort of melanoma patients,*/ 
/* Clark level as a measrue of the tunor's spread,*/ 
/* Breslow thickness determining the prognosis related a rate of 5-year survival rate,*/ 
/* The survival time (months) defined as from the date of biopsy to the date each patient was last seen,  */ 
...
Data melanoma2;
  ...
  ulcerclark=ulcer*clark;
  clark2=0; clark3=0; clark4=0; clark5=0;
  clark2=(clark=2);
  clark3=(clark=3);
  clark4=(clark=4);
  clark5=(clark=5);
run;
proc format;
   value censor 1='Event'
                0='Censored';
run;
proc phreg data=melanoma2; 
   model months*censor(0) = ulcer clark thickness /ties=efron;
   output out=melanoma3 
			dfbeta=dfulcer dfclark dfthickness
            ld=likedisp lmax=maxdisp survival=surv
            resdev=deviance xbeta=Xb
            ressco=scoreulcer scoreclark scorethickness;
		** xbeta contains linear predictor or called predicted scores for each subject;
   title 'Detecting Outliers and Influential Observations';
run;
proc print data=melanoma3 (obs=20); var id months censor dfthickness dfclark dfulcer likedisp maxdisp deviance scoreulcer scoreclark scorethickness; 
run;
**Label the observations by Clinic and by Status;
**The COMPRESS function along with the concatenate symbol* 
**combines the values of Ulcer and Death status and removes any blanks;
data melanoma3;
   set melanoma3;
   ulcerstatus=compress(ulcer||censor);
   *some combinations caused by the missing values in the two variables;
   if ulcerstatus= ".0" or ulcerstatus eq ".1" then delete; 
run;
proc format;
   value $censor 10='Ulcer 1 Censored'
                 11='Ulcer 1 Event'
                 00='Ulcer 0 Censored'
                 01='Ulcer 0 Event';
run;
proc sgplot data=melanoma3;
	scatter y=likedisp x=surv /group=ulcerstatus;
	yaxis label='LD' grid;
	refline 0 /axis=y;
	format ulcerstatus $censor.;
	title 'Assessment Plot of the LD Statistic by Survival Probabilities';
run;
proc gplot data=melanoma3;
   plot likedisp*surv=ulcerstatus / vaxis=axis1;
   axis1 label=(a=-90 r=90 'Likelihood Displacement');
   format ulcerstatus $censor.;
   symbol1 c=red    v=star    i=none;
   symbol2 c=green  v=circle  i=none;
   symbol3 c=yellow v=dot     i=none;
   symbol4 c=blue   v=diamond i=none;
   title h=1.5 'Assessment Plot of the LD Statistic '
               'by Survival Probabilities';
run;
proc sgplot data=melanoma3;
	scatter y=maxdisp x=surv /group=ulcerstatus;
	yaxis lable='L-Max' grid;
	refline 0 /axis=y;
	format ulcerstatus $censor.;
	title 'Assessment Plot of the LMAX Statistic by Survival Probabilities';
run;
proc gplot data=melanoma3;
   plot maxdisp*surv=ulcerstatus / vaxis=axis1;
   axis1 label=(a=-90 r=90 'Likelihood Displacement');
   format ulcerstatus $censor.;
   symbol1 c=red    v=star    i=none;
   symbol2 c=green  v=circle  i=none;
   symbol3 c=yellow v=dot     i=none;
   symbol4 c=blue   v=diamond i=none;
   title h=1.5 'Assessment Plot of the LMAX Statistic '
               'by Survival Probabilities';
run;
proc sgplot data=melanoma3;
	scatter  y=deviance  x=Xb /group=censor;
...
run;
proc sgplot data=melanoma3;
	scatter  y=scorethickness  x=thickness /group=censor;
	format censor censor.;
	yaxis label='Score Residuals of Thickness' grid;
	refline 0 /axis=y;
	title 'Assessment Plot of Score Residuals for Thickness';
run;
proc sgplot data=melanoma3;
	scatter  y=scoreclark  x=clark /group=censor;
	format censor censor.;
	yaxis label='Score Residuals of Clark' grid;
	refline 0 /axis=y;
	title 'Assessment Plot of Score Residuals for Clark';
run;
proc gplot data=melanoma3;
   plot scoreclark*clark=censor / vaxis=axis1;
   axis1 label=(a=-90 r=90 'Score Residuals of Clark');
   format ulcerstatus $censor.;
   symbol1 c=red    v=star    i=none;
   symbol2 c=green  v=circle  i=none;
   symbol3 c=yellow v=dot     i=none;
   symbol4 c=blue   v=diamond i=none;
   title h=1.5 'Assessment Plot of the Score Residuals'
               'for Clark';
run;
*Examine each observation to determine what made the observation influential or an outlier*
*Use PROC RANK to identify the 95th percentile for the distribution of LD and LMAX;
*Proc rank has an ascending ranking for VAR lists by default;
proc rank data=melanoma3 groups=100 out=ranks; *Assign group values from 0 to #group-1;
   var likedisp maxdisp;
   ranks likegroup maxgroup;  *Generate ranks for the corresponding variables;
run;
proc sort data=ranks;
   by descending maxdisp;
run;
proc print data=ranks;
   where likegroup = 99 or maxgroup=99;
   var id months censor surv ulcer clark thickness deviance scoreulcer
       scoreclark scorethickness likedisp dfulcer dfclark dfthickness;
   id maxdisp;
